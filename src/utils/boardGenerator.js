...
function getBaseGameGroupSizes() {
...
export function getExcludedAnimalsForBaseGameMode(allAnimals) {
...
function countAllByCategory(items, category) {
    return items.filter(item => {
        const categories = Array.isArray(item.category) ? item.category : [item.category];
        return categories.includes(category);
    }).length;
}
export function generateBoard(allAnimals, allCoSpecies, options = {}) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (options.requiredAnimals && options.requiredAnimals.length > 0) {
            for (const animalId of options.requiredAnimals) {
                const animal = allAnimals.find(a => a.id === animalId);
                if (animal) {
                    if (animal.level === 1 && board.level1.length < 9)
                        board.level1.push(animal);
                    else if (animal.level === 2 && board.level2.length < 10)
                        board.level2.push(animal);
                    else if (animal.level === 3 && board.level3.length < 5)
        ...
        for (const biome of biomesInPlay) {
            ...
            for (const coSpecies of shuffledCoSpecies) {
                if (board.coSpecies.some(c => c.id === coSpecies.id))
                    continue; // Already added
                const smallCount = board.coSpecies.filter(c => c.size === 1).length;
                if (coSpecies.size === 1 && smallCount >= 5)
                    continue; // Max 5 small co-species
        ...
        const biomeAssignments = new Map();
        const biomeCounts = new Map();
        const level1ByBiome = new Map();
